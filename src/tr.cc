...
QuarkBilinear timerev(QuarkBilinear in, Complex& fac) {
  QuarkBilinear out;
  for (auto l : in.lines) {
    ...
    if (strcmp(c,"D") == 0 || strcmp(c,"U") == 0 || strcmp(c,"S") == 0) {
      {
	std::vector<std::string> l2 = { "GAMMA", "0" };
	out.lines.push_back(l2);
      }
      {
	std::vector<std::string> l2 = { "GAMMA", "1" };
	out.lines.push_back(l2);
      }
      {
	std::vector<std::string> l2 = { "GAMMA", "2" };
	out.lines.push_back(l2);
      }
...
OperatorTerm timerev(OperatorTerm in) {
  OperatorTerm out;
  out.hints = in.hints;
  out.factor = in.factor;
  for (auto& qbi : in.qbi) {
    out.qbi.push_back(timerev(qbi,out.factor));
  }
  return out;
}

Operator timerev(Operator in) {
  Operator out;
  for (auto& t : in.t) {
    out.t.push_back(timerev(t));
  }
  return out;
}
...
int main(int argc, char* argv[]) {
  ...
  timerev(load(argv[1])).write(stdout);

...
QuarkBilinear hc(QuarkBilinear in, Complex& fac) {
  QuarkBilinear out;
  for (auto l : in.lines) {

    const char* c = l[0].c_str();
    if (strcmp(c,"D") == 0 || strcmp(c,"U") == 0 || strcmp(c,"S") == 0) {
      std::vector<std::string> l2 = { "GAMMA", "3" };
      out.lines.insert(out.lines.begin(),l2);
...
OperatorTerm hc(OperatorTerm in) {
  OperatorTerm out;
  out.hints = in.hints;
  out.factor = conj(in.factor);
  for (auto& qbi : in.qbi) {
    out.qbi.push_back(hc(qbi,out.factor));
  }
  return out;
}

Operator hc(Operator in) {
  Operator out;
  for (auto& t : in.t) {
    out.t.push_back(hc(t));
  }
  return out;
}
